...
def load_data():
    ...
    if os.path.exists(BOOKS_FILE) and os.path.getsize(BOOKS_FILE) > 0:
        ...
        # Merge in any missing POPULAR_BOOKS
        existing_keys = set((b['title'], b['author']) for b in st.session_state.books)
...
# Main application after login
    ...
    with tab3:
        ...
        if st.session_state.username in st.session_state.fines:
            ...
            total_fines = sum(f.get("amount", 0) for f in user_fines)
        ...
        if st.session_state.is_admin:
            ...
            with admin_tab4:
                ...
                with col2:
                    ...
                    total_fines = sum(sum(f.get("amount", 0) for f in fines) for fines in st.session_state.fines.values())
